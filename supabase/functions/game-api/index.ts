...
async function finishMatch(match: any, tournament: any, winnerId: string | null, isDraw: boolean, reason: string, winningCells: any = null) {
  ...
  const eloResult = calculateEloPair(
    p1.elo, p2.elo, p1Result,
    p1.rated_games || 0, p2.rated_games || 0,
  );
  const d1 = eloResult.p1Delta;
  const d2 = eloResult.p2Delta;
  const playerUpdate = (player: any, result: number, delta: number) => ({
    ...
    rated_games: (player.rated_games || 0) + 1,
    waiting_since: null,
  });
  const p1Update = playerUpdate(p1, p1Result, d1);
  const p2Update = playerUpdate(p2, p2Result, d2);
  const { data: gameRatings = [] } = await db.from("profile_game_ratings").select("*")
    .eq("game_type", tournament.game_type).in("user_id", [p1.user_id, p2.user_id]);
  const global1 = gameRatings.find((item: any) => item.user_id === p1.user_id) || {
    elo: p1.elo, wins: 0, draws: 0, losses: 0, streak: 0,
  };
  const global2 = gameRatings.find((item: any) => item.user_id === p2.user_id) || {
    elo: p2.elo, wins: 0, draws: 0, losses: 0, streak: 0,
  };
  ...
  await Promise.all([
    ...
    db.from("profile_game_ratings").upsert({ user_id: p1.user_id, game_type: tournament.game_type, elo: p1Update.elo,
      wins: global1.wins + (p1Result === 1 ? 1 : 0), draws: global1.draws + (p1Result === 0.5 ? 1 : 0),
      losses: global1.losses + (p1Result === 0 ? 1 : 0), streak: p1Update.streak,
      updated_at: new Date().toISOString() }, { onConflict: "user_id,game_type" }),
    db.from("profile_game_ratings").upsert({ user_id: p2.user_id, game_type: tournament.game_type, elo: p2Update.elo,
      wins: global2.wins + (p2Result === 1 ? 1 : 0), draws: global2.draws + (p2Result === 0.5 ? 1 : 0),
      losses: global2.losses + (p2Result === 0 ? 1 : 0), streak: p2Update.streak,
      updated_at: new Date().toISOString() }, { onConflict: "user_id,game_type" }),
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!user) return response(req, { success: false, message: "Vui lòng đăng nhập." }, 401);
    ...
    if (action === "create_tournament") {
      if (!["teacher", "admin"].includes(me.role)) {
        return response(req, { success: false, message: "Chỉ giáo viên hoặc quản trị viên được tạo giải đấu." }, 403);
      }
    ...
    if (action === "join_room") {
      ...
      if (!membership) {
        ...
        const rating = await gameRating(user.id, tournament.game_type);
        const { error } = await db.from("tournament_players").insert({
          ...
          rated_games: (rating.wins || 0) + (rating.draws || 0) + (rating.losses || 0),
    ...
    if (!membership && !isTeacher) return response(req, { success: false, message: "Bạn không thuộc giải đấu này." }, 403);
    ...
    if (action === "start_tournament") {
      if (!isTeacher) return response(req, { success: false, message: "Không có quyền." }, 403);
